...
TEST_F(WriterTest, LogToStderrAndFileWritesToFile) {
    std::string path = testFile.string();
    writer::logToStderrAndFile("hello from logger", path);

    auto lines = writer::readAllLines(path);
    ASSERT_TRUE(lines.has_value());
    ASSERT_EQ(lines->size(), 1);
    EXPECT_EQ((*lines)[0], "hello from logger");
}
