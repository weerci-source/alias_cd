...
void initPluginComponents(const PluginStartupInfo *info)
{
    ...
    if (!initRes)
    {
        std::cerr << "Failed to init storage: " << initRes.error().message() << std::endl;
    ...
    if (!loadRes)
    {
        std::cerr << "Failed to load aliases: " << loadRes.error().message() << std::endl;
    }
...
SHAREDSYMBOL void WINAPI EXP_NAME(SetStartupInfo)(const struct PluginStartupInfo *info)
{
    if (!info)
    {
        std::cerr << "SetStartupInfo: info is null" << std::endl;
        return;
    }
    ...
    {
        std::cerr << "Warning: effects not created" << std::endl;
    }
}
...
SHAREDSYMBOL HANDLE WINAPI EXP_NAME(OpenPlugin)(int OpenFrom, INT_PTR Item)
{
    if (!pure::isInitialized() || !actions || !effects)
    {
        ...
        {
            std::cerr << "OpenPlugin: plugin not initialized" << std::endl;
        }
